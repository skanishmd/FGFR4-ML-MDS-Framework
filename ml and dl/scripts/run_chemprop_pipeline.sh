...
OUTPUT_BASE_DIR="model_output/fgfr_finetune_seq_fixed"
mkdir -p "$OUTPUT_BASE_DIR"
mkdir -p logs

TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
LOG_DIR="logs/run_fgfr_seq_fixed_$TIMESTAMP"
mkdir -p "$LOG_DIR"

ulimit -n 4096  # Increase file handle limit for macOS
...
EPOCHS=3            # Must be > warmup epochs (default=2)
BATCH_SIZE=64
NUM_WORKERS=4

# Optional: subset for quick test (uncomment for small dataset)
#FAST_MODE=true
#FAST_NUM_ROWS=10000
...
for TARGET in "${FGFR_TARGETS[@]}"; do
    ...
    # Optional FAST_MODE (small subset)
    # if [ "$FAST_MODE" = true ]; then
    #     OUTPUT_FAST_CSV="gnn_data/chemprop_ready_${TARGET}_fast.csv"
    #     head -n $FAST_NUM_ROWS "$FGFR_CSV" > "$OUTPUT_FAST_CSV"
    #     RAW_CSV="$OUTPUT_FAST_CSV"
    # fi

    OUTPUT_CSV="gnn_data/chemprop_ready_${TARGET}.csv"
    python Python/preprocess_chemprop.py "$RAW_CSV" "$OUTPUT_CSV" "$TARGET"
    ...
    chemprop train \
        ...
        --max-lr 1e-3 \
        --final-lr 5e-4 \
        --save-smiles-splits false || { echo "❌ Fine-tuning failed for $TARGET"; continue; }
...
echo "📁 All logs and predictions saved to: $LOG_DIR"